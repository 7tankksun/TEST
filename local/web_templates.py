    ...
    <body>
      <div class="wrap">
        ...
        <div class="summary">
          ...
          <div class="sum-table-wrap">
            <table class="sum-table">
              <thead><tr><th class="num">순위</th><th>종목명</th><th>티커</th><th>섹터</th><th class="num">총점</th><th>가점(진입/RS/Vol)</th><th>진입일</th></tr></thead>
              <tbody>
                {% for r in top_table %}
                <tr>
                  ...
                  <td>{% if r.score_breakdown %}{{ "%.0f"|format(r.score_breakdown.recency) }}/{{ "%.0f"|format(r.score_breakdown.rs) }}/{{ "%.0f"|format(r.score_breakdown.volume) }}{% else %}-{% endif %}</td>
                  <td>{{ r.entry }}</td>
                </tr>
                {% endfor %}
              </tbody>
