# 읽기 전용 NAS 웹. 빌드: cd kosdaq && docker build -f Dockerfile.nas -t kosdaq-nas-web .
#
# [Synology] 공식 python 이미지 + /app 만 마운트할 때, 실행 명령 예:
#   sh -c "pip install --no-cache-dir 'flask>=3.0' && python /app/app_nas_serve_kosdaq.py"
