...
def _load_sector_map():
    ...
    try:
        ...
        for t, info in CANDIDATES.items():
            _sector_map[t] = info[1] if len(info) > 1 else "기타"
    return _sector_map


def get_sector(ticker):
    return _load_sector_map().get(ticker, CANDIDATES.get(ticker, ["", "기타"])[1])
...
def apply_fundamental_score_layer(new_matches: dict[str, dict]) -> None:
    ...
    기본 적용. 끄려면: SCORE_FUND_DISABLE=1 — SCORE_FUND_MCAP_MAX / SCORE_FUND_OI_MAX 로 상한 조정.
    """
    if not _fund_score_enabled():
        return
    if not new_matches:
        return
    m_mcap = float(os.environ.get("SCORE_FUND_MCAP_MAX", "8"))
    m_oi = float(os.environ.get("SCORE_FUND_OI_MAX", "7"))
    tickers = list(new_matches.keys())
    n_f = len(tickers)
    print(f"  재무 가점 · {n_f}종 · 최대 +{m_mcap + m_oi:.0f}pt")
    raw: dict[str, dict] = {}
    ...
    # 영업: 영업이익률이 한 종목이라도 있으면 전부 마진으로 순위(없는 종은 0점 treated as missing in rank - we need same length; missing margin -> None)
    ...
    for t in tickers:
        ...
        bd["fund_mcap"] = add_m
        bd["fund_operating"] = add_o
        d["score"] = round(base + add_m + add_o, 2)
...
def run_analysis_export(base_dir: str) -> dict:
    ...
    print(
        f"  현재 2단계 종목 전부 수집 | 가점=최근진입+가격RS+거래량·Vol/지수"
        f"{' +재무(시총·영업)' if _fund_score_enabled() else ''} | 추세차트: 상위 {chart_limit if chart_limit else '전체'}종"
    )
    ...
    _default_top_rows = chart_limit if chart_limit > 0 else 50
    summary_table_rows = int(
        os.environ.get("TOP_SUMMARY_ROWS", str(_default_top_rows)).strip() or str(_default_top_rows)
    )
    ...
    payload = {
        ...
        "scoring": {
            "description": "총점=최근진입+가격RS+거래량+Vol/지수+재무(시총·영업, 기본). SCORE_FUND_DISABLE=1로 재무만 끔. SCORE_FUND_MCAP_MAX 등으로 상한 조정. 순위=총점 내림차순.",
