"""
NAS 전용: results_web.json + charts/ 만 읽어 KOSPI Stage2 웹 표시.
(local 앱 app_nas_serve.py 와 동일 패턴, web_templates 공유)

  FLASK_HOST / FLASK_PORT  (기본 포트 8501 — local app.py / Streamlit과 동일)
...
def _ensure_web_templates_path():
    ...
    for d in candidates:
        if os.path.isfile(os.path.join(d, "web_templates.py")):
            ...
            return
    raise ImportError(
        "web_templates.py 없음. kospi 폴더에 복사했는지, 또는 ../local 이 있는지 확인하세요."
    )
...
def _result_json_candidates():
    """export 기본: HERE/nas_web_payload/results_web.json (run_local_export). Docker: PAYLOAD_DIR/payload or 전체/앱."""
...
@app.route("/health")
def health():
    """역프록시·간단 확인(200+경로). 메인 / 가 안 보일 때 먼저 열어 보기."""
    roots, jr = resolve_chart_roots()
    return (
        f"ok kospi-nas json={jr or 'none'} charts={roots}",
        200,
...
def index():
    ...
    html = render_template_string(
        ...
        footer_note=(
            "로컬 kospi/run_local_export.py → Drive 동기화 후 새로고침. "
...
def get_image(filename):
    ...
    app.logger.warning("[NAS KOSPI] PNG 없음: %s (검색: %s)", filename, roots)
    abort(404)
...
if __name__ == "__main__":
    ...
    roots, jr = resolve_chart_roots()
    print(f"[NAS KOSPI] RESULTS_JSON_PATH = {jr}")
    print(f"[NAS KOSPI] chart roots         = {roots}")
    print(f"[NAS KOSPI] http://{host}:{port}")
