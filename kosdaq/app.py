...
def run_analysis_task():
    ...
    global signal_times
    print(f"[{datetime.datetime.now()}] KOSDAQ Stage 2 분석 가동...")
    payload = run_kosdaq_export(BASE_DIR)
    signal_times = {m["ticker"]: m.get("entry", "") for m in payload.get("matches", [])}
    print(f"[{datetime.datetime.now()}] KOSDAQ 분석 완료! (2단계 {len(signal_times)}개)")
...
def index():
    ...
    <body>
        <div class="container">
            <h1> KOSDAQ Stage 2 리포트 ({now_str})</h1>
            <p style="color:#666;font-size:0.9rem;">NAS/동기화: <code>run_local_export.py</code> → <code>app_nas_serve_kosdaq.py</code> (포트 8502 기본)</p>
