...
# 티커 -> 진입일 문자열(또는 True); 웹 표는 keys()만 사용
signal_times = {}


def run_analysis_task():
    """BASE_DIR에 charts/·state/·results_web.json 생성. (run_local_export와 동일 루트)"""
    global signal_times
    print(f"[{datetime.datetime.now()}] KOSPI Stage 2 분석 가동...")
    payload = run_kospi_export(BASE_DIR)
    signal_times = {m["ticker"]: m.get("entry", "") for m in payload.get("matches", [])}
    print(f"[{datetime.datetime.now()}] KOSPI 분석 완료! (2단계 {len(signal_times)}개)")
...
def index():
    ...
    <body>
        <div class="container">
            <h1> KOSPI Stage 2 리포트 ({now_str})</h1>
            <p style="color:#666;font-size:0.9rem;">NAS/동기화: <code>run_local_export.py</code> → <code>app_nas_serve_kospi.py</code> (포트 8501 기본)</p>
            <div style="background:white; padding:20px; border-radius:15px; margin-bottom:30px;">
                <h2 style="margin-top:0;"> 섹터별 요약</h2>
                <table>
                    <tr style="background:#F9FAFB;"><th>섹터</th><th>종목수</th><th>포착 종목명</th></tr>
                    {summary_html if summary_html else "<tr><td colspan='3' style='padding:20px; text-align:center;'>포착된 종목이 없습니다.</td></tr>"}
                </table>
            </div>
            {chart_html}
        </div>
