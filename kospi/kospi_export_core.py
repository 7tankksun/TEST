...
def _write_chart(
    ...
    try:
        ...
        plt.title(f"STAGE 2 ACTIVE: {name} ({ticker})", fontsize=12, fontweight="bold")
...
def run_kospi_export(base_dir: str) -> dict:
    ...
    bench_close, bench_volume, index_status = _load_bench_ks11()
    ...
    for idx, (ticker, info) in enumerate(CANDIDATES.items(), start=1):
        ...
        try:
            m = _compute_kospi_match(ticker, name, bench_close, bench_volume)
            if m is None:
